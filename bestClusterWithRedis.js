...
var redis = require('redis');//var client = require('redis').createClient(process.env.REDIS_URL);
...
if (cluster.isMaster) {
    ...
    for (var i = 0; i < cpuCount; i += 1) {
        cluster.fork();// Listen for dying workers
...
var io = require('socket.io')(serv,{});//this will say that we use sockets
io.set('store', new RedisStore({redisPub:pub, redisSub:sub, store:redisClient}));
